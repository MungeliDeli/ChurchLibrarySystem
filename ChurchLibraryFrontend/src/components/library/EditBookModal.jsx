...
import { updateBook, selectBooksLoading, selectBooksError } from "../../store/slices/bookSlice";
...
const schema = yup.object({
  ...
  bookFile: yup.mixed(), // File is optional on update
}).required();
...
const EditBookModal = ({ isOpen, onClose, book }) => {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      await dispatch(updateBook({ id: book.itemId, data: formData })).unwrap();
      onClose();
    } catch (e) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Book">
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
        ...
        <div>
          <label htmlFor="bookFile" className="block text-sm font-medium text-gray-700">Replace Book File (Optional)</label>
          <input
            {...register("bookFile")}
            type="file"
            id="bookFile"
